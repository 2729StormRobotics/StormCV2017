    ...
    def __filter_contours(input_contours, min_area, min_perimeter, min_width, max_width,
        ...
        if input_contours is not None:
            for contour in input_contours:
                ...
                print(contour.dtype)
                try:
                    tmp_bounding_rect = cv2.boundingRect(contour)
                    if len(tmp_bounding_rect) >= 1:
                        x = tmp_bounding_rect[0]
                    if len(tmp_bounding_rect) >= 2:
                        y = tmp_bounding_rect[1]
                    if len(tmp_bounding_rect) >= 3:
                        w = tmp_bounding_rect[2]
                    if len(tmp_bounding_rect) >= 3:
                        h= tmp_bounding_rect[2]
